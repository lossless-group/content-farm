...
lede: Six docs-derived blockers — doubled command IDs, a description starting with A plugin for Obsidian, and no network-use disclosure.
summary: In-progress punch list generated by re-reading the canonical Obsidian submission docs end-to-end and grepping perplexed against every named requirement. Each of the six blockers names the file and line and the exact replacement, and the Verification section gives runnable greps for confirming each fix. Items 7-10 are deliberately held for a later commit, and the three classes that came back clean are recorded so the audit is not repeated.
from: perplexed
from_path: plans/2026-05-02_Submission-Blockers-Punch-List.md
---
<!-- Rolled up from perplexed/context-v/plans/2026-05-02_Submission-Blockers-Punch-List.md. Edit at the source, not here. Re-run `pnpm rollup:sync` to refresh. -->
